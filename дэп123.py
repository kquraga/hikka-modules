...
class DepModule(loader.Module):
    """Игра 'Дэп' - удвой или проиграй"""
    strings = {
        "name": "Дэп",
        "dep_usage": "🚀 Используй <code>.дэп <сумма></code> чтобы начать игру",
    ...
    @loader.command(ru_doc="<сумма> - игра 'Дэп'")
    async def дэп(self, message: Message):
        """[сумма] - игра Дэп"""
    ...
    async def стоп(self, message: Message):
        ...
        game = self._games.get(user_id)
        if not game:
            await utils.answer(message, "Игра не найдена. Начните с команды .дэп")
            return
